 ...
 *
 * @typedef {'awaiting_invoice' | 'overdue' | 'awaiting_whatsapp' | 'completed'} OrderStatus
 */
 ...
 * @property {string} keeperName
 * @property {string} keeperAddress
 */
 ...
 * @property {string} supplierId Which supplier the routing rules picked.
 * @property {string} assignedAt When it was assigned — the 7-minute timer starts here.
 ...
 * @property {string} [turnStartedAt] When the supplier's current 7-minute window
 *   started. Unset on almost every order, where it is simply the assignment;
 *   set when a Direct Debit order comes back from the customer, because the
 *   window then starts again from the moment it landed back in the
 *   supplier's court.
 */
 ...
 * @property {boolean} active Several suppliers can be active at once.
 * @property {OrderType[]} orderTypes The order types this supplier is configured to handle.
 */
 ...
 */
export function routeOrder(orderType, suppliers = SUPPLIERS) {
  return (
 ...
 * @property {string} name
 * @property {Role} role
 * @property {string} [supplierId] Suppliers are scoped to their own orders; admins see everything.
 */
 ...
 */
export function clockStartedAt(order) {
...
export function statusAfterUpload(order) {
...
export function planFor(orderType) {
 ...
 */
export function sampleBankDetails(customerName) {
