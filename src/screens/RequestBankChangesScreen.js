...
export default function RequestBankChangesScreen() {
  ...
  if (!order || !order.bank || !bankAwaitingReview(order)) {
    return (
      <Screen>
        <CustomHeader onBack={() => navigation.goBack()} title="Bank details" />
        <Body>
    ...
    <Screen>
      ...
      <Body>
        ...
        <Banner icon="info" tone="orange" style={styles.banner}>
          The order goes back to the customer and the upload timer pauses until
          they return it.
        </Banner>
        ...
        <Hint icon="info">
          They see this note and the ticked fields, and can correct anything
          else they spot while they are there.
        </Hint>
      </Body>
