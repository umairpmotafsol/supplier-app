...
import { BANK_FIELDS, BANK_FIELD_LABEL } from '../data/mock';
import Icon from './atoms/Icon';

export function BankPanel({ bank, flagged = [] }) {
  return (
 ...
 */
export function BankThread({ notes }) {
  if (notes.length === 0) {
    return null;
  }
  return (
            ...
            <View style={styles.chips}>
              {note.fields.map(field => (
                <View key={field} style={styles.chip}>
                  <Text style={styles.chipText}>{BANK_FIELD_LABEL[field]}</Text>
                </View>
              ))}
            </View>
          ) : null}
